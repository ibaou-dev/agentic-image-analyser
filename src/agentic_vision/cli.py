...
def _build_parser() -> argparse.ArgumentParser:
    ...
    init_config = sub.add_parser(
        "init-config",
        help="Write a default agentic-vision.toml to the current directory",
    )
    init_config.add_argument(
        "--global",
        action="store_true",
        dest="global_config",
        help="Write to ~/.config/agentic-vision/agentic-vision.toml instead",
    )
    init_config.add_argument(
        "--force",
        action="store_true",
        help="Overwrite if a config file already exists",
    )
    _add_json_pretty(init_config)
...
def _cmd_init_config(args: argparse.Namespace) -> int:
    """Write a minimal agentic-vision.toml with sensible defaults."""
    _config_filename = "agentic-vision.toml"
    if getattr(args, "global_config", False):
        target = Path.home() / ".config" / "agentic-vision" / _config_filename
    else:
    ...
    content = (
        ...
        "# Generated by `agentic-vision init-config`\n"
    ...
    _json_out(
        {"status": "ok", "message": f"Config written to {target}", "path": str(target)},
        pretty=_is_pretty(args),
    )
    return 0
